...
- **Optimized Image Storage**: All generated images (previews, mood boards, UI concepts) flow through `server/object-image-service.ts` which creates WebP-optimized variants: thumb (300px), medium (800px), and full (original quality). The UI loads appropriate sizes: vault thumbnails use ?size=thumb, detail views use ?size=medium, and downloads use ?size=full.
